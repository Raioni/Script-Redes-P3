...
function exibir() {
  ...
  echo "------------------------------------"
  echo ' '
  cat -s /tmp/index.html | sed -r 's/$/framengo/g' | tr '\n' '2' |sed 's/<[^>]*>//g' | sed -E 's/(body ?\{.{1,}\})//g' | sed 's/framengo2/\n/g' | more
  read -p 'Pressione qualquer coisa para continuar...'
}
...
function listar_links() {
  ...
  do
    ...
    elif [ "$(echo $linha | grep -Eo '^.')" == '.' ]; then
      link=$(echo $linha | sed -E 's/^.//g')
      echo "$url$link" >> /tmp/links2
    else
      echo "$url2/$linha" >> /tmp/links2
    fi
  done
